...
const CardDeliverStar: FC<Pick<AccordionProps, 'viewBox'>> = ({ viewBox }) => {
  const Icon = Icons['filled-star'];
  const { fill, stroke } = getStrokeAndFillColors();
  return (
    <>
      <Icon
        stroke={stroke}
...
const CardDeliverStarContainer: FC<Pick<AccordionProps, 'viewBox'>> = (
  props
) => {
  return (
